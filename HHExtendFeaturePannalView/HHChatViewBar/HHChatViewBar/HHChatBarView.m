...
-(void)initLeftBarButton{
    ...
    leftContaintsView.backgroundColor = [UIColor whiteColor];
    ...
    self.leftContaintsView.frame = CGRectMake(0, Kmargin, 45, self.frame.size.height-2*Kmargin);
...
//完成右侧菜单按钮父view的初始化和布局
    ...
    self.rightSubButtonView.backgroundColor = [UIColor clearColor];
//通过代理方法 拿到需要添加的子控件数
    NSInteger count = 1;
...
//        NSLog(@"%@ numberOfSectionWithchatBar 获取到字view的个数:%ld  \n",KHHChatBarViewLog,count);
    ...
    self.count = count;
    CGFloat itemWidth = (contetViewWid-count*Kmargin)/count;
...
//      控制二级菜单frame
        ...
        CGFloat width = sender.frame.size.width+Kmargin;
        CGFloat x = sender.center.x - width*0.5+self.leftContaintsView.bounds.size.width;
        CGFloat y  = self.superview.bounds.size.height - (height+self.bounds.size.height);
        //添加二级菜单
